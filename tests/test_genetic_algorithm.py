...
def test_individual_game_fitness():
    ...
    game_hist = [1, 2, 3, 4, 5, 6] # sum = 21, 3/3 par/impar -> -100 + 10 = -90
    assert individual_game_fitness(game_hist, historical_games) == -90
    
    game_perfect = [10, 20, 30, 31, 41, 51] # sum=183, 3/3 par/impar -> 20
    assert individual_game_fitness(game_perfect, historical_games) == 20
...
def test_fitness_function_portfolio_spread():
    ...
    fit_identical = fitness_function(port_identical, historical_games)
    fit_different = fitness_function(port_different, historical_games)
...
def test_mutate_portfolio():
    ...
    # Com taxa 1.0 (100%), deve mudar pelo menos 1 número em CADA jogo
...
def test_run_evolution_portfolio():
    ...
    best_portfolio = run_evolution(historical, pop_size=10, generations=5, mutation_rate=0.1, portfolio_size=5)
